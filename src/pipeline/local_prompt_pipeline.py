...
from src.utils.data_utils import load_test_data, load_yahoo_test
...
if __name__ == "__main__":
    ...
    if use_yahoo:
        print("Using Yahoo test data")
        data = load_yahoo_test(test_file, test_path)
    else:
        print("Using ShaPe test data")
    ...
    for i, template in enumerate([TEMPLATE_V1, TEMPLATE_V2, TEMPLATE_V3]):
        ...
        save_evaluation(evaluations, test_file, save_path, f'yahoo_mistral_template_{i+1}')
        add_predictions_to_data(updated_data, test_file, save_path, predictions, f'yahoo_mistral_template_{i+1}', template=f"_{i+1}")